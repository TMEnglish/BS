...
def bs_initial_frequencies(rates):
    ...
    Parameter `rates` is an instance of class `Rates`. Probability is
    distributed over the fitnesses in `rates.fitness`. The parameters
    of the normal distribution come from Section 5 of Basener and
    Sanford.
    """
    fit = rates.fitness
...
def bs_mutation_probs(rates, beneficial='1e-3'):
    ...
    Parameter `rates` is an instance of class `Rates`. Probability is
    distributed over mutational effects in `rates.effects`. The shape
    and rate parameters of the Gamma distribution are alpha=0.5 and
    beta=500, respectively, as in Section 5 of Basener nd Sanford. The
    weight of the Gamma distribution in the mixture is `beneficial`,
    which Basener and Sanford set to 1e-3.
    """
    nbins = len(rates.effects)
    bin_width = rates.bin_width
    #
    # The bin masses are obtained by differencing the complementary
